...
class LearningRepo
{
    ...
    public function updateSuggestedPrompts(int $sessionId, array $prompts): void
    {
        $clean = array_values(array_filter(array_map(static fn($p) => trim((string) $p), $prompts), static fn($p) => $p !== ''));
        $stmt = $this->db->prepare(
            'UPDATE learning_sessions
             SET suggested_prompts_json = :suggested_prompts_json,
                 updated_at = NOW()
             WHERE id = :session_id'
        );
        $stmt->execute([
            ':session_id' => $sessionId,
            ':suggested_prompts_json' => json_encode(array_slice($clean, 0, 6), JSON_UNESCAPED_SLASHES),
        ]);
    }

    public function completeSession(int $sessionId, string $reason): void
    {
        $stmt = $this->db->prepare(
            'UPDATE learning_sessions
